...
{	
	...
	public static macro function nextPowerOfTwo(i:haxe.macro.Expr, maxBitsize:Int = MAX_BITSIZE) {
		return macro {
				...
				if ($i < 3) $i;
					...
					var bitsize = catpi.util.BitUtil._bitsize($i - 1, $v{maxBitsize >> 1}, $v{maxBitsize >> 1});
	...
	public static macro function bitsize(i:haxe.macro.Expr, maxBitsize:Int = MAX_BITSIZE) {
		...
		return macro ($i < 3) ? $i : catpi.util.BitUtil._bitsize($i, $v{maxBitsize}, $v{maxBitsize});
	}
	...
	public static macro function _bitsize(i:haxe.macro.Expr, n:Int, delta:Int) {
		if (delta == 0)
			...
			return macro {
				if ( ($i >> $v{n}) == 1 ) $v{n + 1};
				else if ( ($i >> $v{n}) < 1 ) catpi.util.BitUtil._bitsize($i, $v{n - delta}, $v{delta});
				else catpi.util.BitUtil._bitsize($i, $v{n + delta}, $v{delta});
			}
		}
	}	
